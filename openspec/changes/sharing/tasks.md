...
- [ ] 2.1 Add `listShares(principal, id): Promise<NodeShares>` to `SharesService`. For `own`:
...
- [ ] 2.2 Add `@Get(':id/shares')` to `NodesController`, returning `NodeShares`. Assert
...
- [ ] 3.1 Add `shares.service.spec.ts` (or extend the existing e2e spec if one exists) covering:
...
- [ ] 4.1 Add to `apps/web/src/api/`: `resolveShare(token)`, `listNodeShares(nodeId)`,
...
- [ ] 4.2 Add TanStack Query hooks: `useNodeShares(nodeId)` (key `['shares', nodeId]`),
...
- [ ] 5.1 Implement `apps/web/src/components/sharing/ShareDialog.tsx` (docs/04 § Flows: Share):
...
- [ ] 5.2 Wire the dialog: Share button in `ListingToolbar` opens it with the selected node's id.
...
- [ ] 6.1 Create `apps/web/src/routes/s.$token.tsx`. On mount: call `resolveShare(token)`. If 401
  ...
  `SharedViewShell`.
- [ ] 6.2 Implement `SharedViewShell`: the same three-pane layout as the authenticated view but:
...
- [ ] 7.1 Implement `SharedWithMeList` in `apps/web/src/components/tree/SharedWithMeList.tsx`:
...
- [ ] 8.1 Write `scripts/validate/sharing.sh` (Bash, `set -euo pipefail`, executable). Asserts:
...
- [ ] 9.1 If the implementation diverges from design.md (e.g. the ancestor-walk strategy changes,
