...
- [ ] 1.1 In `apps/api/src/nodes/nodes.service.ts`, add a cycle check at the top of `move()`: for
...
- [ ] 1.2 Add a unit test in `apps/api/src/nodes/` (or extend an existing spec) covering:
...
- [ ] 2.1 Create `apps/web/src/hooks/useRename.ts` — wraps `PATCH /nodes/:id` with
...
- [ ] 2.2 Create `apps/web/src/hooks/useMove.ts` — wraps `POST /nodes/move` with `useMutation`,
...
- [ ] 2.3 Ensure `apps/web/src/hooks/useDelete.ts` (or extend the existing delete call in
  the toolbar/dialog) invalidates `['children', parentId]` and `['stats', …ancestors]` on `204`.

## 3. Web — Inline rename in NodeRow

- [ ] 3.1 In `apps/web/src/components/` (e.g. `NodeNameCell.tsx`), convert the name cell into an
  ...
  submit (FR-FILE-030).
- [ ] 3.2 Wire the existing `RenameDialog` (used from the toolbar Rename button) through the same
  `useRename` hook so both paths share one mutation (FR-FLDR-020).

## 4. Web — DeleteDialog with stats and share count

- [ ] 4.1 Extend `apps/web/src/components/dialogs/DeleteDialog.tsx` to fetch
  ...
  (BR-030, FR-FILE-040).
- [ ] 4.2 Wire the toolbar Delete action to open `DeleteDialog` for files as well as folders.

## 5. Web — MoveDialog and FolderPicker

- [ ] 5.1 Create `apps/web/src/components/dialogs/MoveDialog.tsx` — a shadcn Dialog wrapping
  ...
  FR-FLDR-040).
- [ ] 5.2 Create `apps/web/src/components/dialogs/FolderPicker.tsx` — a lazy-expanding folder
...
- [ ] 5.3 Wire the toolbar "Move" button to open `MoveDialog` for the selected node.

## 6. Web — Drag-onto-folder

- [ ] 6.1 In `apps/web/src/components/` (listing row and/or `NodeRow`), add `draggable` and
...
- [ ] 7.1 Write `scripts/validate/file-and-folder-operations.sh` to the AGENTS.md contract:
